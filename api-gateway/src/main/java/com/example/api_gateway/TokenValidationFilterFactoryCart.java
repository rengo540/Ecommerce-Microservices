...
@Slf4j
@Component
public class TokenValidationFilterFactoryCart extends AbstractGatewayFilterFactory<TokenValidationFilterFactoryCart.Config> {
    @Autowired
    private JwtUtils jwtUtils;

    public TokenValidationFilterFactoryCart() {
        super(Config.class); // Required
    }

    @Override
    public GatewayFilter apply(TokenValidationFilterFactoryCart.Config config) {
    ...
    private static class TokenValidationFilter implements GatewayFilter, Ordered {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            ...
            try {
                ...
                log.info("Token is valid. User ID:"+claims.getSubject()+ "and id = " + claims.get("id").toString());
